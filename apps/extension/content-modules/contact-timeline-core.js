(() => {
  ...
  function messageIsOutgoingLike(message) {
  ...
  function activeContactThreadScopeSet() {
    ...
    const activeThreadId = canonicalThreadId(
      state.currentThreadIdForReply || state.activeThreadId || ""
    ) || normalize(state.currentThreadIdForReply || state.activeThreadId || "");
    if (activeThreadId) scoped.add(activeThreadId);
    return scoped;
  }

  function messageInActiveThreadScope(message, threadScope = activeContactThreadScopeSet()) {
    const threadId = canonicalThreadId(
      message && message.threadId || ""
    ) || normalize(message && message.threadId || "");
    if (!threadId || !(threadScope instanceof Set) || threadScope.size === 0) return false;
    return threadScope.has(threadId);
  }
  ...
  function normalizeTimelineRowMessageV2(rowMessage, context = {}, source = "row_live", rowOrder = 0) {
    ...
    const mailbox = mailboxCacheKey(msg.mailbox || (isSelf ? "sent" : "inbox"));
    const activeEmail = extractEmail(
    ...
    let threadId = canonicalThreadId(msg.threadId || "") || normalize(msg.threadId || "");
    if (!threadId && outgoingHint) {
    ...
    if (!senderEmail && outgoingHint && activeEmail) {
    ...
    if (outgoingHint && recipientEmails.length === 0 && contactEmail) {
      recipientEmails = [contactEmail];
    }
    if (outgoingHint && contactEmail && senderEmail === activeEmail && !recipientEmails.includes(contactEmail)) {
      recipientEmails = [...recipientEmails, contactEmail];
    }
    ...
    const normalized = normalizeThreadMessageForChat({
      ...
      messageKey: rowKey,
      direction,
      mailboxHint: normalize(msg.mailboxHint || mailbox) || mailbox,
      isSelf,
  ...
  function messageBelongsToContactTimelineV2(message, context = {}) {
    ...
    const threadScope = activeContactThreadScopeSet();
    const messageThreadId = canonicalThreadId(msg.threadId || "") || normalize(msg.threadId || "");
    const threadMissing = !messageThreadId;
    const mailbox = mailboxCacheKey(msg.mailbox || "");
    const senderLooksSelf = Boolean(
      msg.isSelf === true
      || senderEmail === activeAccountEmail
      || isSelfSenderLabel(msg.sender || "")
    );
    const outgoingLike = messageIsOutgoingLike(msg);
    const inThreadScope = messageInActiveThreadScope(msg, threadScope);
    const hasContact = participantSet.has(contactEmail) || senderEmail === contactEmail || inferredContact === contactEmail;
    if (
      outgoingLike
      && senderLooksSelf
      && (inThreadScope || threadMissing)
      && (hasContact || inferredContact === contactEmail || mailbox === "sent")
    ) {
      return true;
    }
    if (!hasContact) {
      traceDrop("no-contact-match", {
        senderEmail,
        inferredContact,
    ...
    if (!activeAccountEmail) {
      if (isGroup) {
        traceDrop("group-without-active-account", {
          senderEmail,
          inferredContact,
          participantCount: participantSet.size,
          hasContact
