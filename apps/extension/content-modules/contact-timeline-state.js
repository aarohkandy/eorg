(() => {
  ...
  function buildActiveContactTimelineV2(options = {}) {
    ...
    let sentCached = Array.isArray(options.sentCached)
      ? options.sentCached
      : (Array.isArray(state.scannedMailboxMessages[mailboxCacheKey("sent")]) ? state.scannedMailboxMessages[mailboxCacheKey("sent")] : []);
    if (sentCached.length === 0) {
      const timelineRows = Array.isArray(state.activeTimelineMessages) ? state.activeTimelineMessages : [];
      ...
      for (const msg of timelineRows) {
        if (!msg) continue;
        if (!(msg.isSelf === true || normalize(msg.direction || "") === "outgoing")) continue;
        const threadId = canonicalThreadId(msg.threadId || "") || normalize(msg.threadId || "");
        if (threadId) {
          if (scopedThreadIds.size > 0 && !scopedThreadIds.has(threadId)) continue;
          backfilledSentCached.push(msg);
          continue;
        }
      ...
      if (backfilledSentCached.length > 0) {
        sentCached = backfilledSentCached;
        const sentCacheKey = mailboxCacheKey("sent");
        const existingSentCache = Array.isArray(state.scannedMailboxMessages[sentCacheKey])
          ? state.scannedMailboxMessages[sentCacheKey]
          : [];
        if (existingSentCache.length === 0 && !Array.isArray(options.sentCached)) {
          state.scannedMailboxMessages[sentCacheKey] = backfilledSentCached.slice();
        }
        logChatDebug("contact-v2:sent-cache-backfill", {
          ...
          persisted: existingSentCache.length === 0 && !Array.isArray(options.sentCached)
        }, {
          throttleKey: `contact-v2:sent-cache-backfill:${normalize(conversationContext.conversationKey || contactEmail || "unknown")}`,
          throttleMs: 600
        });
      }
    }
