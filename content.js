(() => {
  ...
  const createDiagnosticsCoreApi = typeof contentModuleRegistry.createDiagnosticsCoreApi === "function"
    ? contentModuleRegistry.createDiagnosticsCoreApi
    : null;
  if (!createDiagnosticsCoreApi) {
    throw new Error("[reskin] Missing diagnostics core module. Verify content script order.");
  }
  const diagnostics = createDiagnosticsCoreApi({
    state,
    normalize
  });
