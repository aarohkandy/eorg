(() => {
  ...
  function messageBelongsToConversation(message, context = {}) {
    ...
    if (activeThreadId) scopeThreadIds.add(activeThreadId);
    ...
    const inThreadScope = Boolean(
      messageThreadId
      && scopeThreadIds.size > 0
      && scopeThreadIds.has(messageThreadId)
    );
    const threadMissing = !messageThreadId;
    const senderLooksSelf = Boolean(
      message && message.isSelf === true
      || (activeAccountEmail && snapshot.senderEmail === activeAccountEmail)
      || isSelfSenderLabel(message && message.sender ? message.sender : "")
    );
    ...
    if (
      ...
      && (hasContact || inferredContactMatch || messageMailbox === "sent")
    ) {
      logChatDebug("conversation-filter:include-thread-scoped-outgoing", {
        contactEmail,
        activeAccountEmail,
        ...
        threadMissing,
        snapshot: summarizeChatMessageForDebug(message)
      }, { throttleKey: `include-thread-scoped-outgoing:${contactEmail}:${messageThreadId}`, throttleMs: 900 });
      return true;
    }
    if (!hasContact) {
      ...
      traceDrop("drop-no-contact", {
        hasContact,
        inferredContactKey,
        inferredContactMatch,
        outgoingLike,
        senderLooksSelf,
        inThreadScope,
        threadMissing
    ...
    if (!keepAsIncoming && !keepAsOutgoing) {
      ...
      traceDrop("drop-no-account-match", {
        hasContact,
        keepAsIncoming,
        keepAsOutgoing,
        inferredContactKey,
