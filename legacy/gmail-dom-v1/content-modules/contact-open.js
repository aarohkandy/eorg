(() => {
  ...
  function markOpenPaintStages(root, interactionEpoch, perfTraceId = "") {
    ...
    const isOutgoingLike = (message) => Boolean(
      message
      && (
        message.isSelf === true
        || normalize(message.direction || "") === "outgoing"
      )
    );
    const emitSnapshot = () => {
      ...
      for (const msg of timeline) {
        if (isOutgoingLike(msg)) outgoing += 1;
      }
      const incoming = Math.max(0, timeline.length - outgoing);
      logEvent("Q100", {
  ...
  function scheduleContactTimelineDeferredBuild(options = {}) {
    ...
    const diagRunId = normalize(options.diagRunId || state.activeOpenDiagRunId || "");
    const isStale = () => {
    ...
    const run = async () => {
      if (isStale()) {
        logEvent("O190", {
          runId: diagRunId,
          rc: 1,
          rs: "stale-before-start",
          ck: conversationKey
        });
        if (diagRunId && typeof diagEnd === "function") {
          diagEnd(diagRunId, "aborted", {
            rc: 1,
            rs: "stale-before-start"
          });
          if (normalize(state.activeOpenDiagRunId || "") === diagRunId) state.activeOpenDiagRunId = "";
        }
        return;
      }
      if (perfTraceId) {
      ...
      const sentCached = Array.isArray(state.scannedMailboxMessages[mailboxCacheKey("sent")])
        ? state.scannedMailboxMessages[mailboxCacheKey("sent")]
        : [];
      const isOutgoingLike = (row) => Boolean(
        row
        && (
          row.isSelf === true
          || normalize(row.direction || "") === "outgoing"
        )
      );
      ...
      const clickedThreadId = canonicalThreadId(message && message.threadId || "") || normalize(message && message.threadId || "");
      if (clickedThreadId) threadScope.add(clickedThreadId);
      for (const row of seedRows) {
        const rowThreadId = canonicalThreadId(row && row.threadId || "") || normalize(row && row.threadId || "");
        if (rowThreadId) threadScope.add(rowThreadId);
      }
      let effectiveSentCached = sentCached;
      if (effectiveSentCached.length === 0) {
        const activeTimelineRows = Array.isArray(state.activeTimelineMessages) ? state.activeTimelineMessages : [];
        const fallbackThreadId = clickedThreadId || (threadScope.size > 0 ? Array.from(threadScope)[0] : "");
        const backfilledSentCached = [];
        for (const row of activeTimelineRows) {
          if (!isOutgoingLike(row)) continue;
          const rowThreadId = canonicalThreadId(row && row.threadId || "") || normalize(row && row.threadId || "");
          if (rowThreadId) {
            if (threadScope.size > 0 && !threadScope.has(rowThreadId)) continue;
            backfilledSentCached.push(row);
            continue;
          }
          if (!fallbackThreadId) continue;
          backfilledSentCached.push({
            ...(row && typeof row === "object" ? row : {}),
            threadId: fallbackThreadId
          });
        }
        if (backfilledSentCached.length > 0) {
          effectiveSentCached = backfilledSentCached;
          state.scannedMailboxMessages[mailboxCacheKey("sent")] = backfilledSentCached.slice();
          logChatDebug("contact-open:sent-cache-backfill", {
            conversationKey,
            count: backfilledSentCached.length
          }, {
            throttleKey: `contact-open-sent-cache-backfill:${conversationKey || "unknown"}`,
            throttleMs: 500
          });
        }
      }
      const result = await buildContactTimelineFromRowsChunked(context, {
        liveRows,
        inboxCached,
        sentCached: effectiveSentCached,
        ...
        onChunk: (chunk) => {
          if (isStale()) return;
          const processed = Number(chunk && chunk.processedCount || 0);
          const poolCount = Number(chunk && chunk.poolCount || 0);
          const stage = normalize(chunk && chunk.stage || "");
          chunkAgg.pc = Math.max(chunkAgg.pc, processed);
          if (stage === "normalize") chunkAgg.nc = Math.max(chunkAgg.nc, poolCount || processed);
          if (stage === "filter") chunkAgg.fc = Math.max(chunkAgg.fc, poolCount || processed);
          chunkAgg.yc += 1;
          const now = Date.now();
          if (now - Number(chunkAgg.lastEmitAt || 0) < 250) return;
          chunkAgg.lastEmitAt = now;
          logEvent("O150", {
            runId: diagRunId,
            pc: chunkAgg.pc,
            nc: chunkAgg.nc,
      ...
      if (!result || isStale()) {
        logEvent("O190", {
          runId: diagRunId,
          rc: 1,
          rs: !result ? "missing-result" : "stale-after-build",
          ck: conversationKey
        });
        if (diagRunId && typeof diagEnd === "function") {
          diagEnd(diagRunId, "aborted", {
      ...
      state.contactHydrationFallbackTimer = setTimeout(() => {
        ...
        if (isStale()) {
          logEvent("H223", { rc: 1, rs: "stale-before-hydration", runId: diagRunId });
          return;
        }
        if (state.contactChatLoading || state.replySendInProgress) {
          logEvent("H223", {
            rc: 2,
            rs: state.contactChatLoading ? "already-loading" : "reply-send-in-progress",
            runId: diagRunId
          });
          return;
        }
        const timeline = Array.isArray(state.activeTimelineMessages) ? state.activeTimelineMessages : [];
        const hasOutgoing = timeline.some((msg) => (
          msg
          && (
            msg.isSelf === true
        ...
        if (!shouldHydrate) {
          logEvent("H223", {
            rc: 3,
            rs: hasOutgoing ? "outgoing-already-present" : "insufficient-timeline",
            runId: diagRunId
          });
          return;
        }
      ...
      if (diagRunId && typeof diagEnd === "function") {
        diagEnd(diagRunId, "complete", {
          mc: Array.isArray(result.messages) ? result.messages.length : 0,
          tc: Array.isArray(result.threadIds) ? result.threadIds.length : 0
        });
        if (normalize(state.activeOpenDiagRunId || "") === diagRunId) state.activeOpenDiagRunId = "";
      }
    };
    requestAnimationFrame(() => {
      setTimeout(() => {
        run().catch((error) => {
          ...
          if (diagRunId && typeof diagEnd === "function") {
            diagEnd(diagRunId, "failed", {
              rc: 9,
              rs: "deferred-build-error"
            });
            if (normalize(state.activeOpenDiagRunId || "") === diagRunId) state.activeOpenDiagRunId = "";
          }
  ...
  function openContactTimelineFromRowV2(message, root, route = {}, options = {}) {
    ...
    if (!msg) return false;
    const requestedDiagRunId = normalize(options.diagRunId || state.activeOpenDiagRunId || "");
    if (requestedDiagRunId) {
      state.activeOpenDiagRunId = requestedDiagRunId;
    }
    ...
    if (
      ...
      if (requestedDiagRunId && typeof diagEnd === "function") {
        diagEnd(requestedDiagRunId, "aborted", {
          rc: 1,
          rs: "duplicate-open-guard"
        });
        if (normalize(state.activeOpenDiagRunId || "") === requestedDiagRunId) state.activeOpenDiagRunId = "";
      }
      return true;
    }
